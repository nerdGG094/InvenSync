/* Apresentação do InvenSync — cena 3D e animações da tela de boas-vindas.
   Three.js r185 (bolhas + skybox) + GSAP/ScrollTrigger (animações).

   A bolha é DESENHADA NO SHADER, não é uma imagem colada. Cada ponto recebe
   um quadrado na tela e o fragment shader esculpe dentro dele:
     - anel fino e aceso na borda  -> é o que o olho reconhece como bolha;
     - brilho especular deslocado  -> dá o volume de esfera;
     - reflexo secundário embaixo  -> luz rebatida, tira o ar de adesivo;
     - iridescência pelo ângulo    -> a película colorida do gás;
     - miolo quase vazio           -> bolha é oca, não é bolinha pintada.
   Sem fusão aditiva: aditivo vira névoa luminosa, não vidro.

   A subida também é calculada na GPU (o JS só manda o tempo), então milhares
   de bolhas custam quase nada e nenhuma memória é tocada por quadro.

   Regras de sempre: a cena fica ATRÁS do conteúdo e não captura clique;
   `prefers-reduced-motion` reduz o movimento; pausa com a aba oculta; e tudo
   é destruído em `pagehide`.
*/
...
function criarCena(canvas) {
  ...
  const grupos = [
    criarBolhas(CELULAR ? 520 : 1300, { tamMin: 0.02, tamMax: 0.10, velMin: 1.5, velMax: 2.8,
                                        opacidade: 0.5, fontes: 18 }),
    criarBolhas(CELULAR ? 190 : 430, { tamMin: 0.09, tamMax: 0.28, velMin: 2.0, velMax: 3.7,
                                       opacidade: 0.75, fontes: 12 }),
    criarBolhas(CELULAR ? 55 : 120, { tamMin: 0.26, tamMax: 0.62, velMin: 2.6, velMax: 4.8,
...
function animarConteudo() {
  ...
  if (trilho) {
    ...
    document.querySelectorAll('.etapa').forEach((et) => {
      // Com o trilho no meio, entrar pela lateral ficaria torto: sobe.
...
/* 6. Montagem                                                         */
/* ------------------------------------------------------------------ */
const canvas = document.getElementById('introCena');
const { renderer, cena, camera, grupos, alvo, matVidro, cenaVidro, cameraVidro } = criarCena(canvas);
...
function atualizarImpulso(dt) {
  // Velocidade medida no próprio quadro: vale para roda, barra, teclado, toque
  // e scroll suave — e volta sozinha a zero quando a página para.
  ...
  const ganho = alvo > efeito.empurrao ? SOBE_RAPIDO : VOLTA_DEVAGAR;
  efeito.empurrao += (alvo - efeito.empurrao) * Math.min(1, dt * ganho);
}

let limpezasCards = [];
if (!MENOS_MOVIMENTO) {
  timelineHero = animarConteudo();
  limpezasCards = flutuarConteudo();
  gsap.to(camera.position, { y: -3.4, z: 10, ease: 'none',
...
function aoRedimensionar() {
  ...
  // O alvo do pós-processamento tem que acompanhar a tela, senão o vidro
  // fica esticado e a curvatura das laterais sai do lugar.
...
function laco() {
  ...
  // O vidro é ESTÁTICO de propósito: nada nele responde ao tempo nem ao
  // scroll. Quem se move é o que está atrás — as bolhas.
...
function destruir() {
  ...
  limpezasCards.forEach((f) => f());          // listeners de inclinação dos cards
