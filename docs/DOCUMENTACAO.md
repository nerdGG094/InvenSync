...
> · `qrcode` · `pyotp` (TOTP) · `tinytuya` (tomadas inteligentes) · PyQt5 (launcher)
...
flowchart LR
    ...
    subgraph ADMIN["⚙️ Admin"]
        credentials & audit & backups & smartplugs & errors & docs
    end
    subgraph BASE["🔐 Base"]
...
| Admin (Cofre/Auditoria) | `credentials` · `audit` · `errors` | Admin |
...
│   ├── services/               # transversais: assets, audit, exports, imports, mailer,
│   │                           #   monitoring, people, twofa, tuya, plug_scheduler, docs
│   ├── forms/                  # formulários Flask-WTF
│   ├── routes/                 # blueprints (auth … docs, health)
│   ├── templates/              # Jinja2 (base.html + telas por módulo)
│   └── static/                 # style.css, logo, favicons, uploads/ (avatares)
