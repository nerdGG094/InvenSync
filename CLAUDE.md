...
- `/dev` (DEV, admin) — mini-Monday para o time de desenvolvimento (`routes/dev.py`, `repositories/dev_repo.py`, `models/dev.py`, `templates/dev/`). Três modelos: **DevSprint** (planejada/ativa/concluída, objetivo, datas), **DevTask** (board por `status` backlog→todo→doing→review→done, `priority`, `sprint_id`, `assignee_id` → user, `tags`, `code_ref` commit/branch/PR/link, `position`), **DevUpdate** (timeline de atualizações por tarefa — é onde o dev registra o que editou + o commit). Board kanban server-rendered, filtro por sprint e busca; excluir sprint joga as tarefas ao backlog (`SET NULL`). Tabelas via `create_all`.
  ...
  **O board se alimenta dos commits** (`setup/hooks/post-commit` → `dev_commit.py` → `services/dev_commit.py`). Regras: mensagem citando `#12` vira **atualização** da tarefa 12; título igual ao de um card aberto dos últimos 3 dias também vira atualização (é o caso do `git commit --amend`, que roda o hook de novo com outro hash e sem isso duplicaria o card); qualquer outro commit **cria um card** em `DEV_COMMIT_STATUS` (default Revisão), já na sprint ativa, com prioridade e tags deduzidas do cabeçalho convencional (`fix(dvr):` → prioridade alta, tags `fix, dvr`). Merges e `[skip-kanban]` são ignorados, e o mesmo hash nunca entra duas vezes. O dono dos cards é `DEV_COMMIT_USER_EMAIL` (default `ti@refrigerantesjaboti.com.br`, PRIMO SPINELLI) — não o autor do git, de propósito.
...
| `/cftv` | CFTV / DVRs + câmeras (admin) | | `/dev` | DEV board/sprints/backlog (admin) |
