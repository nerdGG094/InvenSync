...
- `audit.py` — `audit.record(action, entity, entity_id, summary)`; best-effort, never raises, used across mutations. `build_query(...)` is what the `/audit` screen paginates (action/entity/user/text/date range) and `acoes_registradas()` fills the action filter from the table, so a new action never stays invisible in the filter.
...
- **Every background collector answers `saude()`, and `/health` reads it.** `monitoring`, `alerts`, `dvr_events`, `printer_monitor` and `plug_scheduler` each keep the wall-clock of their last finished cycle in memory; `/health` turns it into `{enabled, running, ha_segundos, stale}` via `routes/health.py::_coletor`. **A live thread proves nothing** — the DVR listener sat "alive" for 6 days blocked in a `read()` — so `stale` is computed from the cycle clock, tolerating 3 intervals (floor 5 min) and, before the first cycle closes, the process uptime. Disabled by config is **never** `stale`: a field that cries wolf on every install that doesn't use the module is a field people learn to ignore. `backup_scheduler` is deliberately absent — `last_backup.age_hours/stale` measures the *outcome*, which beats the thread's state. **Nothing watches `/health` on a schedule yet**: it is read by the launcher, by `setup\reiniciar.ps1` after a deploy, and by whoever goes looking.
...
- `pagination.py` — `paginate(items, per_page=20)` slices an in-memory list using `?page`/`?per_page` (20/50/100) and returns `(slice, meta)`; render with the `pager` macro in `templates/_macros.html` (uses the `page_url` context helper to preserve filters). **It is for lists already built in Python** (a repo that sorts/aggregates in memory, or a filter that can't be expressed in SQL — see the tickets list, where the user-scope and SLA filters run in Python and slicing earlier would return half-empty pages).
  **A long history is paginated in the database instead**, with `query.paginate(...)` + `pag.per_page_options` (the macro reads either shape). `/audit` and `/cftv/deteccoes` used to fetch a fixed `limit` (400 and 600) and slice that in memory: everything older was unreachable from the screen and the footer still announced the cap as the total — on the detection page 600 rows is under two hours of the 30 days kept. An audit trail is consulted *after* something happened, so the old rows are the point. Both count the filtered set on every page load, which is fine at this size (`dvr_detection`, the biggest table, counts ~215k rows in tens of ms) but is the thing to watch if a table grows an order of magnitude.
- `troca_celular.py` — the **phone swap**: a stock OUT of a phone to someone who already has one offers to return the old one. Stock and Celulares were two registries that never talked, so a swap meant two manual entries and nothing kept them agreeing. The screen asks (`/movements/celular-do-responsavel`, JSON) and the analyst picks one of three outcomes — **back to stock** (IN of 1 unit + device becomes `disponivel`, unassigned), **to maintenance** (device `manutencao`, *no* stock entry) or **kept** (nothing). **Never silent**: an automatic IN would put a phone that is actually broken, or in someone's drawer, into the balance, and that error only surfaces at inventory time. Guards that matter: a **shared** device is never offered (returning it would strip the other users), the posted device id is re-checked against *that person's* devices (a hand-edited id would touch a colleague's phone), the swap is staged in the session so the OUT's commit saves both or neither, and the chosen product is remembered in `MobileDevice.product_id` so each model is mapped once. Phone products are matched like the printer supplies: category **or** product name (`celular`/`smartphone`).
...
- `/dev` (DEV, admin) — mini-Monday para o time de desenvolvimento (`routes/dev.py`, `repositories/dev_repo.py`, `models/dev.py`, `templates/dev/`). **`/dev` é o painel** (`dev.index` → `dev/index.html`): atalhos do módulo (Kanban, Sprints, Backlog, Nova tarefa), KPIs, progresso da sprint ativa, 4 gráficos e as listas de últimas atualizações / cards parados. O **board mudou para `/dev/board`** (o endpoint continua `dev.board`, então nenhum `url_for` quebrou). Três modelos: **DevSprint** (planejada/ativa/concluída, objetivo, datas), **DevTask** (board por `status` backlog→todo→doing→review→done, `priority`, `sprint_id`, `assignee_id` → user, `tags`, `code_ref` commit/branch/PR/link, `position`), **DevUpdate** (timeline de atualizações por tarefa — é onde o dev registra o que editou + o commit). Board kanban server-rendered, filtro por sprint e busca; excluir sprint joga as tarefas ao backlog (`SET NULL`). Tabelas via `create_all`.
  **Layout do board:** as 5 colunas são um flex `flex:1 1 0` que **divide a largura da tela — o board nunca rola para o lado** (um teste trava isso); quem rola é o corpo de cada coluna, na vertical (`.dev-col-body`, `max-height: calc(100vh - 265px)`). Abaixo de 1200px as colunas quebram em linhas em vez de encolher; abaixo de 576px viram uma por linha. Cada coluna recolhe para uma faixa vertical de 40px (botão no cabeçalho, escolha guardada em `localStorage`) devolvendo o espaço às demais.
  **Mover é por botões de ícone no card** (`◀ ▶` para a coluna anterior/seguinte, `✓` para Concluído, `↺` para reabrir), todos submits de um único `<form>` por card com `name="status" value="<slug>"` → `dev.task_move`; o campo `next` preserva filtro/busca no redirect. O card é uma `div` com `stretched-link` no título (não um `<a>` envolvendo form, que era HTML inválido) e a barra de ações fica acima do link via `z-index`.
  **O board se alimenta dos commits** (`setup/hooks/post-commit` → `dev_commit.py` → `services/dev_commit.py`) — e **de mais de um repositório**: o hook instalado no CARREG-LOGI (ou em qualquer outro projeto) chama o `dev_commit.py` *deste* repo, que grava sempre no banco do InvenSync. O hook é um modelo com `@INVENSYNC@`, preenchido na instalação (`setup\instalar_hooks.ps1 -Repo <pasta>` no Windows, `setup/instalar_hooks.sh --repo <pasta>` no Linux), e passa `--repo` com o repositório de onde veio o commit. O **nome do projeto vira a primeira tag** do card (`carreg-logi`, `invensync` — a pasta do InvenSync se chama InventarioAlmox, então há um mapa), e entra também no dedupe por título: dois repos podem ter o mesmo assunto de commit (`CLAUDE.md: ...`) sem ser o mesmo trabalho.
  Regras: mensagem citando `#12` vira **atualização** da tarefa 12; título igual ao de um card aberto dos últimos 3 dias (mesmo projeto) também vira atualização (é o caso do `git commit --amend`, que roda o hook de novo com outro hash e sem isso duplicaria o card); qualquer outro commit **cria um card** em `DEV_COMMIT_STATUS` (default Revisão), já na sprint ativa, com prioridade e tags deduzidas do cabeçalho convencional (`fix(dvr):` → prioridade alta, tags `fix, dvr`). São ignorados: merges, `[skip-kanban]` **no assunto** (procurar na mensagem inteira fazia o commit que documenta a regra ignorar a si mesmo — aconteceu), o mesmo hash duas vezes, e **commits de rotina** (`DEV_COMMIT_IGNORAR`, default `^(backup automático|wip\b|auto-commit)`): o CARREG-LOGI comita um backup automático a cada 15 min e sozinho encheria o board. O dono dos cards é `DEV_COMMIT_USER_EMAIL` (default `ti@refrigerantesjaboti.com.br`, PRIMO SPINELLI) — não o autor do git, de propósito.
  O hook **não pode derrubar um commit**: `dev_commit.py` engole qualquer exceção e sai com 0. E ele **não usa `create_app()`** — ela roda `create_all()` + light migrations + seeds a cada boot (4s), e um hook de git não tem o que fazer mexendo no schema a cada commit; o script monta um app mínimo (config + SQLAlchemy + import de todos os modelos, necessário para os relacionamentos resolverem) e leva ~2,7s. Instalar: `setup\instalar_hooks.ps1` (Windows) ou `setup/instalar_hooks.sh` (Linux/macOS) — `.git/hooks` não é versionado, cada clone roda uma vez. Desligar: `DEV_COMMIT_ENABLED=0`.
  **O hook é multiplataforma, e isso custou cards.** Editando pelo compartilhamento SMB a partir do Linux, o hook gravado no Windows não achava interpretador, avisava no stderr e saía 0 — o commit passava e o card nunca nascia (foi o que deixou #9/#10/#29/#31/#32 parados no backlog com o trabalho já feito). Três coisas consertam isso e nenhuma é opcional: o caminho do InvenSync cai para **o próprio repo** quando o gravado na instalação não existe (commitando no InvenSync, o `dev_commit.py` está sempre à mão, seja `C:\...` ou `/run/user/.../InventarioAlmox`); os candidatos a interpretador saem do **`uname`**, nunca de `[ -x ]` — montado por SMB, o `python.exe` do Windows aparece como `-rwx` no Linux e a escolha só falharia no `exec`; e o que é da máquina (venv, `INVENSYNC_DIR`, `DB_HOST`) mora em `~/.config/invensync/dev_commit.env`, lido pelo hook. O `.env` do projeto **não** serve para isso: ele vem pelo SMB e o `DB_HOST=127.0.0.1` dele só vale dentro do servidor — de fora, o banco é `192.168.0.54:5432`. A venv do Linux fica **fora do repo** (`~/.local/share/invensync/venv`, criada pelo instalador com o subconjunto de `requirements.txt` que o `dev_commit.py` importa), porque `.venv/` no compartilhamento é a do Windows e `pyvenv.cfg` é um só. Caminhos fixos em `$HOME`, sem `XDG_*`: o snap do VS Code aponta `XDG_DATA_HOME` para dentro do confinamento e o terminal comum não acharia o que ele gravasse lá. Dois testes em `tests/test_dev_commit.py` rodam o hook de verdade e travam as duas primeiras regras.
  **Campos e recursos "estilo Monday"** (todos com teste em `tests/test_dev_monday.py`):
  - **Ordem dentro da coluna** — arrastar para cima/baixo grava `position` 1..N via `dev.task_reordenar`. `dev_repo.reordenar()` **só mexe em cards do mesmo status do card citado na URL**, para um id forjado no payload não remexer outra coluna.
  - **Prazo** (`due_date`, light migration) — chip no card, vermelho **com ícone e tooltip** quando `atrasada` (cor sozinha não carrega aviso); KPI de atrasadas/vencendo em 7d no painel e filtro `?atrasadas=1` no board. Uma tarefa concluída nunca conta como atrasada.
  - **`done_at`** (light migration, com backfill a partir de `updated_at` para os cards já concluídos) — carimbado em `move_task` ao entrar em Concluído e **limpo ao reabrir**, senão o burndown contaria a tarefa como entregue. É o que torna o ciclo médio e o burndown confiáveis; `updated_at` muda a cada edição e não serve.
  - **Checklist** (`DevChecklistItem`, `create_all`) — subtarefas com barra de progresso no card e na página da tarefa; `cascade` apaga junto com o card.
  - **Comentário rápido no card** — botão abre um textarea que posta no mesmo `dev.task_update` por `fetch` (JSON quando vem `X-Requested-With: fetch`); sem JS o form envia normal. A timeline é a mesma do commit.
  - **Filtros do board**: responsável (inclui "sem responsável"), projeto, prioridade e "só atrasadas", somados à sprint e à busca. Todos entram no `next` dos botões de mover, para o filtro sobreviver ao redirect.
  - **Vínculo com chamado** (`DevTask.ticket_id` → `ticket`, `ON DELETE SET NULL`) — badge com link na tarefa e bloco "No board DEV" no chamado, **só para admin**: o módulo DEV é admin-only e um link que dá 403 é pior que link nenhum.
  - **Aviso por e-mail** ao definir responsável (`_avisar_responsavel`, via `mailer.notify_user`) — não avisa quem se atribuiu, não repete se o responsável não mudou, e é best-effort (SMTP fora do ar não derruba a rota).
  **Painel (`dev_repo.estatisticas()`):** KPIs (total, em andamento, atualizações no período, ciclo médio) + 4 gráficos, cada um com a forma escolhida pelo trabalho do dado, não por gosto — **atividade por dia** (linha, série única, dias vazios preenchidos para não mentir sobre o intervalo), **tarefas por coluna** (barra horizontal de uma cor só: 5 categorias nominais comparando grandeza; pintar cada barra de uma cor inventaria categorias e codificaria o comprimento duas vezes), **por projeto** (rosca categórica com a **cor presa ao nome** — a ordem muda com a contagem, e colorir por índice enganaria quem aprendeu "kiox é verde") e **por prioridade** (rampa ordinal, escala ordenada). A sprint ativa ganha um **burndown**: duas séries de mesma unidade (tarefas restantes), logo **um eixo só**; a linha ideal é a reta do total até zero e a real **para no dia de hoje** (`null` no futuro, `spanGaps:false`) em vez de despencar e fingir que a sprint acabou. Usa a **mesma paleta validada** dos outros painéis e o `chart-tabela.js` — as regras de cor estão na seção *Charts* acima e valem aqui. O "projeto" sai de `dev_repo.PROJETOS` (lista explícita): sem ela, um card com tags `bug, dev` viraria um projeto chamado *bug*. O **ciclo médio** mede criação → `done_at` (ver o bullet de `done_at` acima): só entram cards concluídos que têm o carimbo, e reabrir um card o tira da conta. A primeira versão usava `updated_at` e era só aproximada — `updated_at` muda a cada edição, então um card antigo que alguém renomeia hoje "levaria" semanas para ficar pronto.
  **Avatares no card:** o macro `dev_avatar` mostra o **responsável** e, sobreposto e esmaecido, **quem criou** a tarefa (só quando é outra pessoa) — foto de `User.photo` (`uploads/avatars/`) ou as iniciais, no padrão do resto do app; o nome completo fica no `title`. Por isso `dev_repo.board()` faz `joinedload` de `assignee`/`created_by`/`sprint`: sem isso cada card custava 3 consultas.
  **Arrastar e soltar** entre colunas (HTML5 DnD): o card muda de coluna na hora e o servidor é avisado por `fetch` no mesmo `dev.task_move` — que responde **JSON quando o header `X-Requested-With: fetch` chega**, em vez de re-renderizar o board inteiro a cada card arrastado; se o POST falhar a página recarrega para voltar ao estado real. Depois de arrastar, o JS reescreve os botões `data-mv` do card (`STATUS` vem do servidor via `statuses|tojson`) — senão as setas continuariam apontando para os vizinhos da coluna antiga. Os botões de ícone **não são redundantes**: DnD não existe em toque nem por teclado.
...
| `/cftv` | CFTV / DVRs + câmeras (admin) | | `/dev` | DEV painel + `/dev/board` kanban (admin) |
...
## Deploy
`atualizar.bat` (repo root): `git pull` → `pip install -r requirements.txt` → boot-check (`create_app()`) → **restart via `setup\reiniciar.ps1`**. The restart used to be a printed reminder; it is now a step, because the reminder was the single source of production failure — **12 of the 42 entries in the `/errors` log** are `Could not build url for endpoint '…'` (all of them `dev.index`), i.e. new templates running on the old process — the single largest cause after the cookie collision below, which accounts for 26 more. Measured 17/09/2026; the log has had no new entry since 01/09. The boot-check runs *before* anything is killed, so a broken pull leaves the old version serving.
